...
document.addEventListener("click", (e) => {
  if (navMenu.classList.contains("banner__nav--open") && !navMenu.contains(e.target)) {
    closeNav();
  }
});
...
document.addEventListener("keyup", (e) => {
  if (e.key === "Tab" && navMenu.classList.contains("banner__nav--open")  && !navMenu.contains(document.activeElement)) {
    closeNav();
  }
...
const checkScroll = () => {
  ...
  if (window.scrollY > bannerHeight && window.matchMedia("(min-width: 50rem)").matches) {
